...
export function ChangesList({
  ...
  if (changes.files.length === 0)
    return <p className="text-sm text-neutral-500">Working tree clean.</p>;
      ...
      <div className="space-y-0.5">
        {changes.files.map((f) => (
          <button
            ...
            className="flex w-full items-center gap-3 rounded-md px-1 py-1 text-left font-mono text-xs hover:bg-white/5"
          >
            <span className="w-16 shrink-0 text-neutral-500">{f.status}</span>
            <span className="min-w-0 flex-1 truncate text-neutral-200 hover:text-sky-300">
              {f.path}
            </span>
