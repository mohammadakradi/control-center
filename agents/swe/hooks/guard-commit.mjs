...
// Enforces engineering rule 6 ("never commit/push directly to the default branch").
...
if (hasCommit && onDefault) {
  block(
    `[swe] Blocked: refusing to commit directly to the default branch "${branch}". ` +
      `Create a feature branch first (e.g. \`git switch -c <name>\`), then commit. ` +
      `(swe engineering rule 6)`,
  );
}
...
for (const { segment } of pushSegments) {
  ...
  if (targetsDefault || barePushOnDefault) {
    ...
    block(
      `[swe] Blocked: refusing to ${how} to the default branch ("${branch || "main/master"}"). ` +
