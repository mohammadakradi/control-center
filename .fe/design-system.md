...
| `MobileTopBar` / `MobileTabBar` | `components/MobileNav.tsx` | — | Below `md`: a slim sticky top bar (brand + theme icon) and a fixed bottom tab bar. Layout `<main>` carries `pb-24` to clear the tab bar. Tabs are `flex-1 min-w-0` with a `truncate` label. **Now at 6 tabs** (Tasks landed 2026-08-11): each track is ~53px at 320px, so "Dashboard"/"Projects"/"Settings" ellipsize on the narrowest phones — accepted, because the icon carries recognition and the full label is still the link's accessible name. **Six is the ceiling**: a 7th (the planned Backlog page) needs a real overflow affordance — an iOS-style "More" tab, or icon-only below `sm` — not a shorter word. |
...
| `nav-links` | `components/nav-links.tsx` | `NAV_LINKS`, `isActive()` | Single source of truth for the primary nav items (Dashboard · Agents · Projects · **Tasks** · Usage · Settings), shared by sidebar and mobile nav. Add a route here and both navs pick it up. `isActive` matches by prefix, so `/tasks/<id>` keeps Tasks lit. **Six entries is the mobile tab bar's ceiling** — see the `MobileTabBar` row before adding a seventh. |
...
| `ProjectFilterNav` | `components/ProjectFilterNav.tsx` | `projects: {id,name,count}[]`, `selected: string \| null` | Project filter for `/tasks`. **Links, not buttons**, for `SpendRangeNav`'s reasons (URL state → server component, bookmarkable, `aria-current="page"` is the honest ARIA for navigation). **Wrapping pills** rather than a fixed segmented control, because the project count is unbounded — a segmented bar would overflow or squeeze names to nothing. Only projects with ≥1 task are offered (a filter to a guaranteed-empty list is a dead end). Per-pill count is `aria-hidden` with an `sr-only` "…, N tasks" beside it — a bare number reads as part of the name. **Self-guards**: returns `null` below two projects, so callers don't each have to remember there's no choice to make. |
